...
export default function AITools() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      import("../services/api").then(m => m.default.get("/transactions/ai-usage/?page_size=10")),
    ])
      .then(([tr, ur]) => {
  ...
  return (
    <div>
      ...
      <div className="card" style={{
        background: "linear-gradient(135deg,rgba(196,98,45,0.08),rgba(139,92,246,0.08))",
        ...
        <div className="aitools-impact-grid">
          {[
        ...
        {tools.map(tool => (
          <div key={tool.id} className="card" style={{ position: "relative", overflow: "hidden" }}>
            ...
            <div style={{ display: "flex", gap: 12, alignItems: "flex-start", marginBottom: 12 }}>
              <div style={{ fontSize: 24, flexShrink: 0 }}>{TYPE_ICONS[tool.tool_type] || "🤖"}</div>
      ...
      {usage.length > 0 && (
        <>
          ...
          {/* Desktop table */}
          <div className="table-wrap aitools-table-desktop">
            <table>
              <thead>
                <tr>
                  <th>Tool</th><th>Candidate</th><th>Score</th>
                  <th>Notes</th><th>Used By</th><th>Date</th>
                </tr>
              </thead>
              <tbody>
                {usage.map(u => (
                  <tr key={u.id}>
                    ...
                    <td>
                      {u.score_generated
                        ? <span style={{ color: "var(--accent)", fontWeight: 700 }}>{u.score_generated}%</span>
                        : "—"}
                    </td>
          ...
          <div className="aitools-usage-mobile">
            {usage.map(u => (
              <div key={u.id} className="card card-sm" style={{ marginBottom: 10 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                  <span style={{ fontWeight: 700, fontSize: 13 }}>{u.tool_name}</span>
                  {u.score_generated && (
                ...
                <div style={{ fontSize: 12, color: "var(--text-secondary)", marginBottom: 4 }}>
                  <strong>Candidate:</strong> {u.candidate_name || "—"}
                </div>
                {u.output_notes && (
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginTop: 6, fontSize: 11.5, color: "var(--text-muted)" }}>
                  <span>{u.used_by_name || "System"}</span>
